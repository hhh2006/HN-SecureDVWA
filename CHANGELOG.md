...
## V8 — Security Intelligence & UX Fixes

- Fixed sidebar re-open behavior after collapse.
- Fixed navigation list scrolling and mobile drawer behavior.
- Added Security Health Center, Threat Timeline, Risk Matrix, Security Controls, Assurance Studio, and print-ready Security Report.
- Added animated radar/signal/orbit visuals and responsive security-intelligence layouts.
- Added explicit scoring and risk heuristics without fabricating telemetry.
...
## v2.0.2 — Responsive Command Center
- Rebuilt the application shell with CSS Grid instead of legacy float layout.
- Main navigation + content now occupy the viewport horizontally on desktop.
- Content width is fluid with min-width:0 safeguards to prevent broken overflow.
- Dashboard keeps cards and analytics side-by-side at desktop widths and collapses progressively on smaller screens.
- Security Log uses an internal scroll area so the entire dashboard does not become unnecessarily tall.
- Removed forced page-level `<br><br>` spacing after the main body.
- Added responsive viewport metadata to help/source windows.
